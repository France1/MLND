...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5, maxQ=0.0):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == True:
            ...
            # linearly decaying epsilon with rate delta
            # next_epsilon = self.epsilon-self.delta
            # exponentially decaying epsilon with rate a
            next_epsilon = math.exp(-self.a*self.n_trials)
            if next_epsilon < 0:
                self.epsilon = 0.0
            else:
    ...
    def createQ(self, state):
        ...
        if self.learning:
            # add state in self.Q if key is not already in self.Q
            if state not in self.Q:
                # print('state from createQ', state)
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            p = random.random()
            # select with epsilon probability where p is between 0 and 1
            if p <= self.epsilon:
                action = random.choice(self.valid_actions)
            # equivalent to 1-epsilon probability
            else:
                # list of actions to select
                actions = []
                self.maxQ = self.get_maxQ(state)
                for a, q in self.Q[self.state].iteritems():
                    if q == self.maxQ:
                        # action = a
                        # To modify: randomly select actions that ties
                        actions.append(a)
